...
def separate(sensordata):
    ...
    Returns:
        ...
        upcast: upcast data
    """
    max_depth_ind = np.argmax(sensordata.Depth)
    downcast = sensordata.iloc[:max_depth_ind, :]
    upcast = sensordata.iloc[max_depth_ind:, :]
...
def resample(sensordata, interval=0.25):
    ...
    data_is_aggregrated = True # whether data have been aggregated
    meanRange = []

    if min(np.abs(np.diff(depth))) <= 2 * interval:
        data_is_aggregrated = False # the data is not aggregated
        meanRange = [(depth >= d-interval) * (depth<=d+interval) >0 for d in new_depth]

    # interpolate / resample for each feature on the new depth grid
    for i in range(sensordata.shape[1]):
        if sum(~sensordata.iloc[:,i].isnull()) < 1: # no data for ith feature
            new_sensordata[:, i+1] = np.nan
        else:
            if data_is_aggregrated: # if data are already aggregated, do interpolation
                new_sensordata[:, i+1] = np.interp(new_depth, depth, sensordata.iloc[:, i])
            else:
                values = np.array(sensordata.iloc[:, i])
                new_sensordata[:, i+1] = [np.mean(values[meanRange[j]]) for j in range(len(new_depth))]

    return pd.DataFrame(new_sensordata, columns= ["Depth"] + sensordata.columns.values.tolist())


def transimission_to_BAT(transmission):
    """
    function to change transsmission to BAT
    """
    return -np.log(transmission/100)*4
    
def cond_to_speccond(conductitivty, temperature):
    """
    function to conductivity to specific conductivity
    """
    return conductitivty / (1+0.02*(temperature - 25))

def smooth(data, config):
    """
...
def preprocess(data,config):
    ...
    filtered_data = smooth(pre_data_resample, config)
