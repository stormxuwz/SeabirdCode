...
def gauss_function(x, a, x0, sigma,y0,k):
	return a*np.exp(-(x-x0)**2/(2*sigma**2))+y0+k*(x-x0)

def fitGaussian(x,y,x_mean,weight= None):
	if weight is None:
		popt, pcov = curve_fit(lambda x,a,sigma,k: gauss_function(x,a,x_mean,sigma,max(y)-a,k), x, y,maxfev=3000)
	else:
		popt, pcov = curve_fit(lambda x,a,sigma,k: gauss_function(x,a,x_mean,sigma,max(y)-a,k), x, y, sigma=weight,maxfev=2000)
