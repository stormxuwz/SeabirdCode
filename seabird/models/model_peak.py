...
class peak(object):
	...
	def featureExtraction(self):
		...
		for shape in self.shape_fit:
			...
			allPeaks["peakIndex"].append(peakIndex)

			allPeaks["leftIndex"].append(max(0, shape["left_index"][0],int(peakIndex - 3* (shape["left_std"]))))
			allPeaks["rightIndex"].append(min(len(self.x)-1,shape["right_index"][-1],int(peakIndex + 3* (shape["right_std"]))))
